...
import { CheckCircle2, Circle, IndianRupee, RefreshCw, ShieldCheck, UserRound } from "lucide-react";
...
// Must match entity.enums.WorkflowStage on the backend exactly - this stepper was
// previously built against invented stage names ("INTAKE", "OCR_REVIEW", "VALIDATION")
// that don't exist there, so real cases never matched any step and the whole "Case
// progress" stepper silently showed nothing as complete regardless of actual progress.
// CANCELLED/ARCHIVED are terminal exception states, not steps on the happy path, so
// they're intentionally left out of this linear stepper.
const stages = ["CREATED", "DOCUMENTS_PENDING", "DOCUMENTS_UPLOADED", "DOCUMENTS_VERIFIED", "CA_ASSIGNED", "UNDER_REVIEW", "READY_TO_FILE", "PAYMENT_PENDING", "PAYMENT_COMPLETED", "PROCESSING", "FILED", "COMPLETED"];
...
export default function CaseDetailsPage() {
 ...
 const missingQuery = useQuery({ queryKey: ["case-missing", id], queryFn: () => CaseService.missingDocuments(id), enabled: Number.isFinite(id) });
  ...
  const currentStage = useMemo(() => stages.indexOf(caseQuery.data?.workflowStage ?? "CREATED"), [caseQuery.data?.workflowStage]);

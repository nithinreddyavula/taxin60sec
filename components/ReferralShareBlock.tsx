...
export default function ReferralShareBlock({
  ...
  async function copyLink() {
    try {
      ...
      // clipboard can fail on older browsers - link is still visible below either way
    }
  }
  ...
  return (
    <section className="card-dark p-6">
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-white/[.03] p-3">
        <span className="truncate text-sm font-mono text-blue-200">{referralCode}</span>
        {referredCount !== undefined && (
          <span className="shrink-0 text-xs text-secondary">{referredCount} referred so far</span>
        )}
      </div>

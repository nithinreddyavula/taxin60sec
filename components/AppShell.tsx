...
export default function AppShell({ children, roles = ["ROLE_CLIENT", "ROLE_CA", "ROLE_ADMIN"] }: { children: React.ReactNode; roles?: string[] }) {
  ...
  return <div className="min-h-screen bg-[#020817] lg:grid lg:grid-cols-[250px_1fr]">
    ...
    <div className="min-w-0"><header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-3 border-b border-white/10 bg-[#020817]/90 px-4 backdrop-blur lg:px-8"><button onClick={() => setOpen(true)} className="text-slate-300 lg:hidden" aria-label="Open menu"><Menu /></button><div className="hidden items-center gap-2 text-sm text-secondary sm:flex"><ShieldCheck size={17} className="text-emerald-400" /> Secure Tax60 workspace <ChevronRight size={15} /></div><div className="flex flex-1 items-center justify-end gap-3"><Link href="/" className="hidden items-center gap-1.5 text-sm font-semibold text-slate-300 hover:text-emerald-400 sm:flex" aria-label="Back to Tax60 home"><Home size={16} /> Home</Link><GlobalSearch /><Link href="/profile" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400" aria-label="Open profile"><UserRound size={18} /></Link></div></header><main className="app-shell-content mx-auto max-w-7xl p-4 text-slate-100 sm:p-6 lg:p-8">{children}</main></div>
  </div>;
}
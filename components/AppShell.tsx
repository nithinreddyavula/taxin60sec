...
export default function AppShell({ children, roles = ["ROLE_CLIENT", "ROLE_CA", "ROLE_ADMIN"] }: { children: React.ReactNode; roles?: string[] }) {
  ...
  return <div className="min-h-screen bg-[#020817] lg:grid lg:grid-cols-[260px_1fr]">
    <aside className={`fixed inset-y-0 left-0 z-50 w-72 border-r border-white/10 bg-[#060d1d] p-4 text-white transition-transform lg:static lg:w-auto ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      <div className="flex items-center justify-between px-2 py-3"><Link href="/" title="Back to Tax60 home" className="flex items-center gap-2 text-xl font-bold"><ShieldCheck size={20} className="text-emerald-400" />Tax<span className="text-emerald-400">60</span></Link><button onClick={close} className="lg:hidden" aria-label="Close menu"><X /></button></div>
      ...
      <div className="absolute inset-x-4 bottom-4 rounded-xl border border-white/10 bg-white/[.03] p-3"><p className="truncate text-sm font-semibold text-white">{user.fullName ?? user.email ?? "Tax60 member"}</p><p className="mt-1 text-xs text-secondary">{user.roles[0]?.replace("ROLE_", "")}</p><button onClick={() => { logout(); router.replace("/"); }} className="mt-3 flex items-center gap-2 text-sm text-slate-300 hover:text-white"><LogOut size={16} /> Sign out</button></div>
    </aside>
    {open && <button onClick={close} aria-label="Close menu overlay" className="fixed inset-0 z-40 bg-black/50 lg:hidden" />}
    <div className="min-w-0"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/10 bg-[#020817]/90 px-4 backdrop-blur lg:px-8"><button onClick={() => setOpen(true)} className="text-slate-300 lg:hidden" aria-label="Open menu"><Menu /></button><div className="hidden items-center gap-2 text-sm text-secondary sm:flex"><ShieldCheck size={17} className="text-emerald-400" /> Secure Tax60 workspace <ChevronRight size={15} /></div><Link href="/profile" className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-400" aria-label="Open profile"><UserRound size={18} /></Link></header><main className="mx-auto max-w-7xl p-4 text-white sm:p-6 lg:p-8">{children}</main></div>
  </div>;
}